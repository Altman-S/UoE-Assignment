  ...
  def task4(l1: List[TitleBasics], l2: List[TitleCrew], l3: List[NameBasics]): List[(String, Int)] = {
    ...
    val name_num_list = l3.map(i => {
      if (i.knownForTitles.isDefined && i.primaryName.isDefined) {
        if (i.knownForTitles.get.size >= 2) i.knownForTitles.get.map(j => {
          if (tconst_start_map.contains(j)) (i.primaryName.get, 1)
          else (i.primaryName.get, 0)
        })
        else List()
      }
      else List()
    ...
    val reduce_list = name_num_list.map(i => i.reduce((a, b) => (a._1, a._2 + b._2)))

    val result = reduce_list.flatMap(i => {
      if (i._2 >= 2) List((i._1, i._2))
      else List()
    })
